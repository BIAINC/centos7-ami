...
yum install -y coreutils grep

DEVICE=/dev/xvdf
ROOTFS=/rootfs
RELEASE_RPM=https://mirrors.edge.kernel.org/centos/7.5.1804/os/x86_64/Packages/centos-release-7-5.1804.el7.centos.x86_64.rpm
ENA_VER=1.5.3

# Update both of the below together
IXGBEVF_VER=4.3.5
IXGBEVF_URI=https://astuteinternet.dl.sourceforge.net/project/e1000/ixgbevf%20stable/4.3.5/ixgbevf-4.3.5.tar.gz

export PATH=/usr/sbin:$PATH
mkdir $ROOTFS

echo Initialize the block device
...
rpm --root=$ROOTFS -ivh --nodeps $RELEASE_RPM
...
chroot $ROOTFS yum -y install cloud-init cloud-utils-growpart gdisk dkms make perl gcc kernel-devel \
  auditd perl curl wget net-tools vim-minimal bzip2 bind-utils selinux-policy-targeted  dracut-fips
...
firewalld postfix linux-firmware mariadb-libs prelink
...
LABEL=var     /var                xfs     defaults,relatime,nodev               0 2
...
sysfs         /sys                sysfs   defaults,nodev                        0 0
proc          /proc               proc    defaults,nodev                        0 0
END
...
GRUB_CMDLINE_LINUX="crashkernel=auto console=ttyS0,115200 console=tty0 net.ifnames=0 biosdevname=0 audit=1 nousb"
...
echo Installing IXGBEVF Drivers
...
mount|grep 'rootfs' |awk '{print $3}'|sort -r|sudo xargs -L 1 umount
